...
2. `**count = signal(...)**` - use `**kindling.signal**` (re-exported from `kindling`) inside `app.reactive`, not `signals.signal` directly (unless you intentionally use the [pysignals](https://github.com/monster0506/pysignals) API for shared state outside the block, as in **routes**).
